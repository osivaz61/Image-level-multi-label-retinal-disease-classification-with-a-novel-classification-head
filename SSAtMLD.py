...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.05):
        super().__init__()
...
class Attention_Cls(nn.Module):
    ...
    def forward(self, tgt, x, H, W):
        B, N, C = x.shape
        q = self.q(tgt).reshape(B, tgt.shape[1], self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        if self.sr_ratio > 1:
                x_ = x.permute(0, 2, 1).reshape(B, C, H, W)
                x_1 = self.act(self.norm1(self.sr1(x_).reshape(B, C, -1).permute(0, 2, 1)))
                x_2 = self.act(self.norm2(self.sr2(x_).reshape(B, C, -1).permute(0, 2, 1)))
                ...
                v1 = v1 + self.local_conv1(v1.transpose(1, 2).reshape(B, -1, C//2).
                                        transpose(1, 2).view(B,C//2, H//self.sr_ratio, W//self.sr_ratio)).\
                    view(B, C//2, -1).view(B, self.num_heads//2, C // self.num_heads, -1).transpose(-1, -2)
                x1 = (attn1 @ v1).transpose(1, 2).reshape(B, tgt.shape[1], C//2)
                attn2 = (q[:, self.num_heads // 2:] @ k2.transpose(-2, -1)) * self.scale
                attn2 = attn2.softmax(dim=-1)
                attn2 = self.attn_drop(attn2)
                v2 = v2 + self.local_conv2(v2.transpose(1, 2).reshape(B, -1, C//2).
                                        transpose(1, 2).view(B, C//2, H*2//self.sr_ratio, W*2//self.sr_ratio)).\
                    view(B, C//2, -1).view(B, self.num_heads//2, C // self.num_heads, -1).transpose(-1, -2)
                x2 = (attn2 @ v2).transpose(1, 2).reshape(B, tgt.shape[1], C//2)
...
class Block_Cls(nn.Module):
    ...
    def forward(self, tgt, x, H, W):
        ...
        tgt = tgt + self.drop_path(tgt)
        tgt = self.norm1(tgt)
        tgt2 = self.attn(tgt, x, H, W)
        tgt = tgt + self.drop_path(tgt2)
        tgt = self.norm2(tgt)
        return self.norm3(self.mlp(tgt, H, W))
...
##     bs = 2
##     query_embed = nn.Embedding(8, 1024)
##     query_embed.requires_grad_(False)
##     tgt = query_embed.weight.unsqueeze(0).expand(bs, -1, -1)
##     classifier = Block_Cls(dim = 1024, num_heads = 4, mlp_ratio = 2, qkv_bias = True, qk_scale = True, 
##                            drop = 0.05, attn_drop = 0.0, drop_path = 0.05, norm_layer = partial(nn.LayerNorm, eps=1e-6), sr_ratio = 4).cuda()
##     output = torch.randn(2, 144, 1024).cuda()
##     output = classifier(tgt.cuda(), output, 12, 12)